  ...
  */
  ...
  levels = [
    ...
    new Level("Monsters!", ["Jump on enemies", "heads to kill them!"], [
      ...
      "############################################"
        ...
        `
        Monster0 _direction: (1)|
        Monster1 _direction: (0)|
        `
    ...
    new Level("Staircase", ["...Sometimes", "", "paths are simpler", "than we may think..."], [
      ...
      "#######   @  ##     "
        ...
        `
        Monster0 _direction: (1)|
        Monster1 _direction: (1)|
        Monster2 _direction: (1)|
        `
  ...
  new Level("The Pillars", ["...Sometimes", "", "we must", "find creative ways", "to resolve", "our problem..."], [
    ...
    "####    ####    ####"
      ...
      `
      Monster0 _direction: (0)|
      Monster1 _direction: (0)|
      Monster0 _patrol: (7, 11) (12, 11)|
      Monster1 _patrol: (15, 11)|
      `
  ...
  */
  ...
  if(loading_level && in_title) {
    ...
    if((screen.height == levels[level - 1].game_components.length * 20) && (screen.width == levels[level - 1].game_components[0].length * 20) && keys[13]) {
      ...
      screen.text(levels[level - 1].title, screen.width / 2, screen.height / 4.5, "white", (0.125 * screen.height).toString(), "courier", "bold");
      for(var count = 0; count < levels[level - 1].captions.length; count++) {
        
        screen.text(levels[level - 1].captions[count], screen.width / 2, screen.height / 1.5 + (count * (0.0625 * screen.height)) - ((levels[level - 1].captions.length * (0.0625 * screen.height)) / 2), "white", (0.0625 * screen.height).toString(), "courier");
